...
<template>
	<div class="content2">
		<headerNav :pageTitle="title" :goBackShow="goBackShow" @headRightClick="deleteAddress" :isRightShow="isRightShow" :rightColor="color" :rightValue="rightValue"></headerNav>
		...
		<mt-popup class="popup-wrapper" v-model="popupVisible" position="bottom">
			<div class="piker-wrapper">
				...
				<mt-picker :slots="slots" @change="onValuesChange" :visibleItemCount="visibleCount" :itemHeight="itemHeight" class="slot1" :valueKey="valueKey"></mt-picker>
			</div>
		</mt-popup>
...
<script>
	...
	export default {
		...
		created() {
			this.getQueryData();
			this.fromUrl = window.sessionStorage.getItem('fromUrl');
			console.log(window.sessionStorage.getItem('fromUrl'));
		},
		mounted() {
